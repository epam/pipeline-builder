...
SQUOTE
  : '\'' -> pushMode(SquoteInterpolatedString)
  ;

DQUOTE
  : '"' -> pushMode(DquoteInterpolatedString)
  ;
...
SQuoteCommandStart
  : ('${' | '~{' ) -> pushMode(DEFAULT_MODE) , type(StringCommandStart)
  ;

SQuoteUnicodeEscape
  : '\\u' (HexDigit (HexDigit (HexDigit HexDigit?)?)?)? -> type(StringPart)
  ;
...
EndHereDocCommand
  : '>>>' -> mode(DEFAULT_MODE), type(EndCommand)
  ;
...
MetaSquote
  : '\'' -> pushMode(MetaSquoteString)
  ;

MetaDquote
  : '"' -> pushMode(MetaDquoteString)
  ;
...
MetaLbrack
  : '[' -> pushMode(MetaArray), pushMode(MetaValue)
  ;
...
MetaSquoteUnicodeEscape
  : '\\u' (HexDigit (HexDigit (HexDigit HexDigit?)?)?)? -> type(MetaStringPart)
  ;

MetaEndSquote
  : '\'' ->  popMode, type(MetaSquote), popMode
  ;
...
MetaDquoteUnicodeEscape
  : '\\u' (HexDigit (HexDigit (HexDigit HexDigit?)?)?) -> type(MetaStringPart)
  ;

MetaEndDquote
  : '"' ->  popMode, type(MetaDquote), popMode
  ;
